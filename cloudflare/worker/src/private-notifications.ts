...
const readLostFoundModeratorNotifications = async (
  env: PrivateNotificationsEnv,
  userId: string,
) => {
  const rows = await requireD1(env).prepare(
    `SELECT id, receiver_id, NULL AS actor_id, type, content, link, is_read, created_at
       FROM lost_found_moderator_notifications
      WHERE receiver_id = ?
