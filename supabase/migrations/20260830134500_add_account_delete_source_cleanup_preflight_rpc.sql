-- Validate the complete source-cleanup contract in one server-side operation.
-- This keeps the account-delete Worker below the platform subrequest limit
-- while remaining fail-closed for every current source resource.
...
BEGIN
  ...
  LOOP
    IF to_regclass(format('public.%I', source_table)) IS NULL
      ...
      RAISE EXCEPTION USING
        ERRCODE = '55000',
        MESSAGE = 'ACCOUNT_DELETE_SOURCE_SCHEMA_UNAVAILABLE';
    END IF;
  END LOOP;
  ...
  IF to_regprocedure('public.delete_practice_pro_access_for_account_cleanup(uuid)') IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'ACCOUNT_DELETE_SOURCE_CLEANUP_RPC_UNAVAILABLE';
  END IF;
END;
$$;
