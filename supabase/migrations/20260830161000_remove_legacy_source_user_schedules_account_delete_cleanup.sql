-- Private schedules are authoritative in D1.  In source-control mode d1,
-- public.user_schedules is intentionally write-fenced, so it is not an
-- account-delete source resource and must not be probed or mutated here.
