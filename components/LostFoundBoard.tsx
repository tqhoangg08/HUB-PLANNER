...
// --- SUBMIT MODAL COMPONENT (MOVED OUTSIDE) ---
...
const SubmitModal: React.FC<SubmitModalProps> = ({ isOpen, onClose, type, onShowToast }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 3. Success
            onShowToast("Đăng tin thành công! Tin của bạn đang chờ kiểm duyệt và sẽ hiển thị sớm.", 'success');
            onClose();

        } catch (err: any) {
            console.error(err);
            onShowToast("Lỗi khi đăng tin: " + err.message, 'error');
        } finally {
            ...
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-scaleIn">
                <div className={`p-4 text-white flex justify-between items-center shrink-0 ${type === 'FOUND' ? 'bg-[#003375]' : 'bg-[#990000]'}`}>
                    <h3 className="font-bold text-lg flex items-center gap-2">
                        {type === 'FOUND' ? <PlusCircle size={20}/> : <Megaphone size={20}/>}
                        {type === 'FOUND' ? 'Đăng tin Nhặt được đồ' : 'Đăng tin Báo mất đồ'}
                    </h3>
                ...
                <form onSubmit={handleSubmit} className="p-6 overflow-y-auto custom-scrollbar space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-bold text-gray-700 mb-1">{type === 'FOUND' ? 'Khu vực nhặt' : 'Khu vực mất'} <span className="text-red-500">*</span></label>
...
// --- DETAIL MODAL COMPONENT (MOVED OUTSIDE) ---
        ...
        <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
            <div className="bg-white w-full max-w-6xl h-[90vh] md:h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row relative animate-scaleIn">
                    ...
                    <div className="p-6 space-y-4">
                        <div>
                            ...
                            <div className="flex items-center gap-2 text-gray-500 text-sm mt-2">
                                <Calendar size={14} />
                                <span>Ngày đăng: {new Date(item.created_at).toLocaleDateString('vi-VN')}</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 pt-4 border-t border-gray-200">
                             <div className={`flex items-center gap-3 p-3 rounded-xl border ${item.type === 'FOUND' ? 'bg-blue-50 border-blue-100' : 'bg-red-50 border-red-100'}`}>
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${item.type === 'FOUND' ? 'bg-blue-200 text-[#003375]' : 'bg-red-200 text-[#990000]'}`}>
                                    {item.type === 'FOUND' ? <User size={20} /> : <HelpCircle size={20} />}
                                </div>
                                <div>
                                    <span className="block text-xs text-gray-500 font-bold uppercase">{item.type === 'FOUND' ? 'Người nhặt' : 'Người mất'}</span>
                                    <span className="font-bold text-gray-800 text-lg">{item.user_name}</span>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3 p-3 rounded-xl border border-green-200 bg-green-50">
                                ...
                                <div>
                                    ...
                                    <span className="font-bold text-green-700 text-lg">{item.contact_info}</span>
                                </div>
                            </div>
                ...
                <div className="w-full md:w-[60%] h-[55%] md:h-full bg-white flex flex-col relative z-0">
                    <CommentSection 
                        contextId={`lost_found_${item.id}`} 
                        title={item.type === 'FOUND' ? 'Trao đổi nhận đồ' : 'Manh mối / Hỏi thăm'}
