...
import { Trash2, Plus, Star, Search, X, Pencil, BookOpen, Crown, TrendingUp, Loader2, AlertCircle, ChevronRight, BarChart2, ChevronLeft, Award, ArrowUpDown, ArrowUp, ArrowDown, ListFilter } from 'lucide-react';
...
// --- COMPONENT NHẬP ĐIỂM (ĐÃ SỬA CHẶT CHẼ) ---
const ScoreInput = ({ 
  value, 
  onChange 
}: { 
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 2. Chặn ký tự không phải số hoặc dấu chấm bằng Regex
    // Chỉ cho phép số và tối đa 1 dấu chấm (ví dụ: 9.5)
    if (!/^\d*\.?\d*$/.test(newVal)) return;

    const parsed = parseFloat(newVal);

    // 3. Chặn NaN và khoảng giá trị 0 - 10
  ...
  return (
    <input 
      ...
      onKeyDown={(e) => {
        // Chặn các ký tự đặc biệt của input number: e, E, +, -
...
export const SemesterTable: React.FC<SemesterTableProps> = ({ semester, index, onUpdateSemester, onRemoveSemester }) => {
  ...
  // State quản lý sắp xếp: null (mặc định) | 'desc' (cao -> thấp) | 'asc' (thấp -> cao)
  ...
  const handleSortToggle = () => {
      playClick();
      setSortOrder(prev => {
          if (prev === null) return 'desc'; // Bấm lần 1: Giảm dần (điểm cao lên đầu)
          if (prev === 'desc') return 'asc'; // Bấm lần 2: Tăng dần
          return null; // Bấm lần 3: Về mặc định
      });
  };
  ...
  const processedSubjects = useMemo(() => {
      // 1. Filter
      let result = semester.subjects.filter(subject => 
  ...
  return (
    <div className={`mb-8 bg-white rounded-xl shadow-sm border overflow-visible transition-all duration-300 hover:scale-[1.01] hover:shadow-xl ${hasData ? 'border-opacity-100' : 'border-gray-200'}`}>
      {/* Header with Stats */}
      <div className={`px-6 py-4 border-b flex flex-col md:flex-row md:items-center justify-between gap-4 ${headerColor} rounded-t-xl`}>
        ...
        {/* Stats Bar */}
        <div className="flex flex-wrap items-center gap-2 md:gap-4 text-sm relative z-10">
              {/* Rank Badge Dropdown */}
              {hasData && (
      ...
      {semester.subjects.length > 0 && (
          <div className="px-6 py-2 bg-gray-50/50 border-b border-gray-100 flex flex-wrap gap-2 justify-end items-center">
            
            {/* 👇 NÚT SẮP XẾP RA NGOÀI ĐÂY */}
            <button 
                onClick={handleSortToggle}
                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-medium transition-all active:scale-95 ${sortOrder 
                    ? 'bg-blue-50 border-blue-200 text-[#003375] shadow-sm' 
                    : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                title="Sắp xếp theo điểm thang 10"
            >
                {sortOrder === 'desc' ? (
                    <>
