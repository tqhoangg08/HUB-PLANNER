...
const SEED_FILE = path.join(CACHE_DIR, 'admin-events-seed.sql');
...
const values = rows.map((row) => [
  ...
  sqlText(row.contribution_link),
  sqlText(row.contributor_note),
  sqlText(row.section),
  sqlText(row.score),
...
const statements = [
  ...
  "DELETE FROM sync_metadata WHERE resource = 'admin_events';",
];
...
console.log(
  `Seeded ${rows.length} admin events into ${
